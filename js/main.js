...
function init() {
    ...
    document.getElementById('btn-save-nickname').addEventListener('click', () => {
        ...
        // Sprawdź auto-join z URL
        if (window._autoJoinRoom) {
            showMenuPane('menu-join');
            document.getElementById('input-room-code').value = window._autoJoinRoom;
            document.getElementById('join-status-msg').innerText =
                currentLang === 'pl' ? 'Kod pokoju wczytany z linku! Kliknij "Dołącz".' :
                    (currentLang === 'es' ? 'Código cargado del enlace. Haz clic en "Unirse".' :
                        'Room code loaded from link! Click "Join".');
    ...
    document.getElementById('btn-copy-invite').addEventListener('click', () => {
        if (!window._inviteUrl) return;
        navigator.clipboard.writeText(window._inviteUrl).then(() => {
            const txt = document.getElementById('invite-link-text');
            const orig = txt.textContent;
            txt.textContent = currentLang === 'pl' ? 'Skopiowano! ✓' : (currentLang === 'es' ? '¡Copiado! ✓' : 'Copied! ✓');
            setTimeout(() => { txt.textContent = orig; }, 2000);
        }).catch(() => {
            // Fallback: prompt
            prompt(currentLang === 'pl' ? 'Skopiuj ten link:' : 'Copy this link:', window._inviteUrl);
        });
    });
    ...
    document.querySelectorAll('#btn-cancel-lobby, #btn-disconnect-client, #btn-cancel-join').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            window._inviteUrl = null;
            showMenuPane('menu-main-buttons');

            // Wyczyść parametr room z URL
            const cleanUrl = window.location.origin + window.location.pathname;
            window.history.replaceState({}, '', cleanUrl);
        });
    });

    // === AUTO-JOIN Z URL (?room=XXXXX) ===
    const urlParams = new URLSearchParams(window.location.search);
    const autoRoom = urlParams.get('room');
    if (autoRoom && autoRoom.length === 5) {
        // Poczekaj aż nickname zostanie ustawiony i przejście do menu
        const waitForMenu = () => {
            const menuScreen = document.getElementById('menu-screen');
            if (menuScreen && menuScreen.classList.contains('active')) {
                showMenuPane('menu-join');
                document.getElementById('input-room-code').value = autoRoom.toUpperCase();
                document.getElementById('join-status-msg').innerText =
                    currentLang === 'pl' ? 'Kod pokoju wczytany z linku! Kliknij "Dołącz".' :
                        (currentLang === 'es' ? 'Código cargado del enlace. Haz clic en "Unirse".' :
                            'Room code loaded from link! Click "Join".');
            } else {
                // Nickname screen is active — wait
                window._autoJoinRoom = autoRoom.toUpperCase();
            }
        };
        waitForMenu();
    }
