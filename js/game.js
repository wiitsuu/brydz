...
class BridgeGame {
    constructor(isClient = false) {
        ...
        // Guardy przeciw duplikatom łańcuchów AI
        this._aiBiddingTimer = null;
        this._aiPlayTimer = null;
    ...
    startNewRound() {
        ...
        if (!this._isHumanControlled(this.currentPlayer)) {
            this._scheduleAIBidding(600);
        }
    }
    ...
    playerBid(bid) {
        ...
        if (currentPos !== this.myPosition) return; // Tylko lokalny gracz klika

        bid.player = this.myPosition;
    ...
    _processAIBidding() {
        ...
        const currentPos = this.biddingManager.currentPlayer;
        if (this._isHumanControlled(currentPos)) {
    ...
    _scheduleAIBidding(delay = 600) {
        if (this._aiBiddingTimer) clearTimeout(this._aiBiddingTimer);
        this._aiBiddingTimer = setTimeout(() => {
            this._aiBiddingTimer = null;
            this._processAIBidding();
        }, delay);
    }

    _scheduleAIPlay(delay = 600) {
        if (this._aiPlayTimer) clearTimeout(this._aiPlayTimer);
        this._aiPlayTimer = setTimeout(() => {
            this._aiPlayTimer = null;
            this._processAIPlay();
        }, delay);
    }
    ...
    getPlayableCards() {
        ...
        if (!this.trickManager) return [];

        const activePlayer = this.trickManager.currentPlayer;

        // Na kliencie _isHumanControlled zawsze zwraca false, więc obsługujemy oddzielnie
        if (this.isClient) {
            // Klient może grać swoimi kartami lub kartami dziadka jeśli jest rozgrywającym
            if (activePlayer === this.myPosition ||
                (activePlayer === this.dummy && this.declarer === this.myPosition)) {
            ...
            if (this.trickManager.allComplete) {
                ...
                setTimeout(() => {
                    ...
                    if (!this._isHumanControlled(trick.winner)) {
                        this._scheduleAIPlay(600);
                    }
                }, 3500);
            }
        } else {
            // Natychmiast wyślij stan z zaktualizowanym currentPlayer do klientów
            this.currentPlayer = this.trickManager.currentPlayer;
            this._emitStateChange();
    ...
    updateStateFromHost(statePayload) {
        ...
        this.contract = statePayload.contract;
